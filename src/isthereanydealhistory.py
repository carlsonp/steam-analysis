	...
	try:
		...
		for i,check_id in enumerate(to_check):
			...
			try:
				...
				r = requests.get(requests.Request('GET', "https://api.isthereanydeal.com/games/info/v2", params=params).prepare().url, timeout=30)
				if (r.ok):
					data = r.json()
					...
					r_second = requests.get(requests.Request('GET', "https://api.isthereanydeal.com/stats/waitlist/v1", params=params).prepare().url, timeout=30)
					if (r_second.ok):
